...
const dijkstra = (start) => {
  ...
  for (let i = 0; i < n - 1; i++) {
    ...
    graph[now].forEach(([dest, dist]) => {
      const cost = distance[now] + dist;
      distance[dest] = Math.min(distance[dest], cost);
    });
  }
};
...
distance.forEach((dist) => console.log(dist === Infinity ? "INF" : dist));
